...
In my experience, web UI console is timeseries based which means you will missing some data if run load testing for serveral hours due to it does not make sense for keeping browser for long period.
 ...
 In this task, I did not really use distributed mode as testing frmework since the RPS is a bit low. I only run test on local machine but I can give some screenshot as evidence for above desicription
 ...
 BTW, if you are interested in how to setup for this part I can show you in the interview if time is possible.
...
## Screenshot of test result for demo project
 - You can reference folder "result-screenshot-example" for CI and Peformance test result
    ...
    - locust_xxx.png: recent load testing project that against gRPC microservices.
...
- Max RPS is 97 (For both store and query api)
    - Store user behavior data
    - Query user behavior data
- Simulate 100 users and hatching rate in 0.1, total request is 5245 in 1 mins and max response time would be 160 ms
```bash
